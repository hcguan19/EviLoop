...
from browser_use.tools.control_state import (
	control_value_matches,
	read_control_state,
	validate_commit_click,
	validate_control_mutation,
	wait_for_control_state,
)
from browser_use.tools.control_state import (
	inspect_controls as inspect_live_controls,
)
from browser_use.tools.control_state import (
	set_control_value as set_live_control_value,
)
...
from browser_use.tools.views import (
	...
	SetControlsByLabelsAction,
	SetFormValuesAction,
	StructuredOutputAction,
	SwitchTabAction,
	TransferControlValueAction,
	UploadFileAction,
	WaitForUiAction,
)
...
class Tools(Generic[Context]):
	def __init__(
		...
		completion_guard: Callable[[BrowserSession, str], str | None | Awaitable[str | None]] | None = None,
	):
		...
		async def _click_by_index(
			...
			try:
				...
				before_state = None
				try:
					before_state = await read_control_state(browser_session, node, params.index)
					...
					if commit_validation.is_commit and not commit_validation.allowed:
						payload = commit_validation.model_dump(mode='json')
						return ActionResult(
							error=(
								f'Commit blocked: {commit_validation.reason} '
								f'Details: {json.dumps(payload["invalid_controls"], ensure_ascii=False)}'
							),
							long_term_memory=f'Commit was blocked: {commit_validation.reason}',
							metadata={'commit_validation': payload},
						)
				except Exception as validation_error:
					logger.debug(f'Control pre-click validation unavailable: {validation_error}')
				...
				if before_state is not None:
					try:
						await asyncio.sleep(0.05)
						after_state = await read_control_state(browser_session, node, params.index)
						if after_state is not None:
							transition = {
								'before': before_state.model_dump(mode='json'),
								'after': after_state.model_dump(mode='json'),
							}
							metadata['control_state_transition'] = transition
							if (
								before_state.type == 'checkbox'
								and before_state.checked == after_state.checked
								and not before_state.disabled
							):
								return ActionResult(
									error='Checkbox click did not change its checked state.',
									long_term_memory='The checkbox click was not applied; use inspect_controls before retrying.',
									metadata=metadata,
								)
							if before_state.type in {'checkbox', 'radio'}:
								memory += (
									f' Verified checked state: {before_state.checked} -> {after_state.checked}.'
								)
					except Exception as verification_error:
						logger.debug(f'Control post-click verification unavailable: {verification_error}')
		...
		async def input(
			...
			try:
				...
				if not mutation_validation.allowed:
					return ActionResult(
						error=f'Control mutation blocked: {mutation_validation.reason}',
						long_term_memory=mutation_validation.reason,
						metadata={'mutation_validation': mutation_validation.model_dump(mode='json')},
					)
			except Exception as validation_error:
				logger.debug(f'Control mutation validation unavailable: {validation_error}')
		...
		@self.registry.action(
			'Inspect live values and validity of page controls. Use before complex form work, after uncertain actions, '
			'or before committing. Returns checkbox/radio state, selected options, slider values, text values, and validity.',
			param_model=InspectControlsAction,
		)
		async def inspect_controls(params: InspectControlsAction, browser_session: BrowserSession):
			states = await inspect_live_controls(
				browser_session,
				visible_only=params.visible_only,
				max_controls=params.max_controls,
			)
			if params.index is not None:
				states = [state for state in states if state.index == params.index]
			if params.type:
				control_type = params.type.strip().lower()
				states = [
					state
					for state in states
					if control_type in {state.type.lower(), state.tag.lower(), (state.role or '').lower()}
				]
			payload = [state.model_dump(mode='json') for state in states]
			result = json.dumps({'controls': payload, 'count': len(payload)}, ensure_ascii=False)
			return ActionResult(
				extracted_content=result,
				long_term_memory=f'Inspected {len(payload)} live controls and their verified states.',
				metadata={'control_count': len(payload)},
			)

		@self.registry.action(
			'Set a control to an exact value and verify it by DOM readback. Prefer this for sliders, checkboxes, radio '
			'buttons, native selects, number inputs, and contenteditable fields when ordinary click/input is ambiguous.',
			param_model=SetControlValueAction,
		)
		async def set_control_value(params: SetControlValueAction, browser_session: BrowserSession):
			node = await browser_session.get_element_by_index(params.index)
			if node is None:
				return ActionResult(error=f'Control index {params.index} is unavailable; refresh browser state.')
			try:
				mutation_validation = await validate_control_mutation(browser_session, node)
				if not mutation_validation.allowed:
					return ActionResult(
				...
				before, after = await set_live_control_value(
					browser_session,
					node,
					params.value,
					params.index,
				...
				return ActionResult(error=f'Failed to set control {params.index}: {error}')
			if after is None:
				return ActionResult(error=f'Control {params.index} could not be read back after mutation.')

			desired = params.value
			verified = control_value_matches(after, desired)
			...
			if not verified:
				return ActionResult(
					error=(
						f'Control {params.index} readback did not match requested value. '
						f'Observed: {json.dumps(after.model_dump(mode="json"), ensure_ascii=False)}'
					),
					long_term_memory='A deterministic control update failed verification; inspect controls before retrying.',
					metadata={'control_state_transition': transition},
				)
			message = (
				f'Set control {params.index} to {desired!r} and verified live value/state '
				f'{after.value!r}.'
			)
			return ActionResult(
		...
		@self.registry.action(
			'Set visible checkboxes or radio buttons by exact accessible labels and verify every state by DOM readback. '
			'Pass the complete requested label list in one call; this avoids unstable numeric element grounding.',
			param_model=SetControlsByLabelsAction,
		)
		async def set_controls_by_labels(params: SetControlsByLabelsAction, browser_session: BrowserSession):
			def normalize(value: str) -> str:
				return ' '.join(value.casefold().split())

			states = await inspect_live_controls(browser_session, visible_only=True, max_controls=250)
			toggle_states = [state for state in states if state.type in {'checkbox', 'radio'} and state.index is not None]
			resolved = []
			missing = []
			ambiguous = []
			for requested_label in params.labels:
				label = normalize(requested_label)
				matches = [
					state
					for state in toggle_states
					if label and label in {normalize(state.name), normalize(state.context)}
				]
				if not matches:
					missing.append(requested_label)
				elif len(matches) > 1:
					ambiguous.append(
						{'label': requested_label, 'indices': [state.index for state in matches]},
					)
				else:
					resolved.append((requested_label, matches[0]))
			if missing or ambiguous:
				return ActionResult(
					error='Exact labelled-control matching failed; no controls were changed.',
					metadata={'missing_labels': missing, 'ambiguous_labels': ambiguous},
				)

			transitions = []
			for requested_label, state in resolved:
				node = await browser_session.get_element_by_index(state.index)
				if node is None:
					return ActionResult(
						error=f'Control for label {requested_label!r} became unavailable; refresh browser state.',
						metadata={'completed': transitions},
					)
				before, after = await set_live_control_value(
					browser_session,
					node,
				...
				verified = after is not None and control_value_matches(after, params.checked)
				transitions.append(
					{
				...
				if not verified:
					return ActionResult(
						error=f'Control {requested_label!r} failed checked-state verification.',
						metadata={'completed': transitions},
					)
			message = f'Set and verified {len(transitions)} controls by exact visible labels.'
			return ActionResult(
				extracted_content=message,
				long_term_memory=message,
				metadata={'labelled_control_transitions': transitions},
			)

		@self.registry.action(
			'Transfer the exact current value of one visible control into another control and verify the target by DOM '
			'readback. Use for copy, paste, duplicate, or exact-value transfer tasks without regenerating the value.',
			param_model=TransferControlValueAction,
		)
		async def transfer_control_value(params: TransferControlValueAction, browser_session: BrowserSession):
			source = await browser_session.get_element_by_index(params.source_index)
			target = await browser_session.get_element_by_index(params.target_index)
			if source is None or target is None:
				return ActionResult(error='Source or target control is unavailable; refresh browser state.')
			try:
				source_state = await read_control_state(browser_session, source, params.source_index)
				if source_state is None or source_state.value is None:
					return ActionResult(error=f'Source control {params.source_index} has no readable value.')
				if source_state.type == 'password' or source_state.value == '<redacted>':
					return ActionResult(error='Password values cannot be transferred.')
				if isinstance(source_state.value, list):
					return ActionResult(error='Source control produced an unsupported composite value.')
				before, after = await set_live_control_value(
					browser_session,
					target,
					source_state.value,
				...
				return ActionResult(error=f'Control value transfer failed: {error}')
			verified = after is not None and control_value_matches(after, source_state.value)
			metadata = {
				'source': source_state.model_dump(mode='json'),
				'target_before': before.model_dump(mode='json') if before else None,
				'target_after': after.model_dump(mode='json') if after else None,
				'verified': verified,
			}
			if not verified:
				return ActionResult(
					error=f'Target control {params.target_index} failed exact transfer verification.',
					metadata=metadata,
				)
			message = (
				f'Transferred the exact visible value from control {params.source_index} '
				f'to control {params.target_index} and verified it.'
			)
			return ActionResult(
				extracted_content=message,
				long_term_memory=message,
		...
		@self.registry.action(
			'Set several form controls in one bounded batch, verify every value by DOM readback, and optionally '
			'validate all visible required controls. Use for compact forms after inspect_controls.',
			param_model=SetFormValuesAction,
		)
		async def set_form_values(params: SetFormValuesAction, browser_session: BrowserSession):
			transitions = []
			for assignment in params.assignments:
				node = await browser_session.get_element_by_index(assignment.index)
				if node is None:
					return ActionResult(
						error=f'Form batch stopped: control index {assignment.index} is unavailable.',
						metadata={'completed_assignments': transitions},
					)
				try:
					mutation_validation = await validate_control_mutation(browser_session, node)
					if not mutation_validation.allowed:
						return ActionResult(
							error=f'Form batch stopped: {mutation_validation.reason}',
							long_term_memory=mutation_validation.reason,
							metadata={'completed_assignments': transitions},
						)
					before, after = await set_live_control_value(
					...
					return ActionResult(
						error=f'Form batch stopped at control {assignment.index}: {error}',
						metadata={'completed_assignments': transitions},
					)
				verified = after is not None and control_value_matches(after, assignment.value)
				transition = {
				...
				if not verified:
					return ActionResult(
						error=f'Form batch stopped: control {assignment.index} failed DOM readback verification.',
						long_term_memory='A verified form batch stopped before commit because a control value did not match.',
						metadata={'completed_assignments': transitions},
					)
			...
			if params.verify_required:
				states = await inspect_live_controls(browser_session, visible_only=True, max_controls=250)
				invalid_required = [
					state.model_dump(mode='json') for state in states if state.required and not state.disabled and not state.valid
				]
				if invalid_required:
					return ActionResult(
						error=(
							'Form values were set, but required controls remain invalid: '
							f'{json.dumps(invalid_required, ensure_ascii=False)}'
						),
						long_term_memory='Do not commit until all required form controls pass validation.',
						metadata={
							'completed_assignments': transitions,
							'invalid_required_controls': invalid_required,
						},
					)

			message = f'Set and verified {len(transitions)} form controls.'
			return ActionResult(
				extracted_content=message,
				long_term_memory=message,
				metadata={
		...
		@self.registry.action(
			'Honor a temporal constraint, then verify that an optional live control is visible, enabled, and stable. '
			'Use for delayed buttons, dynamically rendered fields, menus, and controls that changed after an action.',
			param_model=WaitForUiAction,
		)
		async def wait_for_ui(params: WaitForUiAction, browser_session: BrowserSession):
			if params.index is None:
				if params.seconds > 0:
					await asyncio.sleep(params.seconds)
				return ActionResult(
					extracted_content=f'Waited {params.seconds:.3f} seconds.',
					metadata={'wait_seconds': params.seconds},
				)
			...
			if node is None:
				return ActionResult(error=f'Control index {params.index} is unavailable; refresh browser state.')
			try:
				first, second = await wait_for_control_state(
					browser_session,
					node,
					params.index,
					seconds=params.seconds,
					require_visible=params.require_visible,
					require_enabled=params.require_enabled,
					stable_ms=params.stable_ms,
				)
			except Exception as error:
				return ActionResult(error=f'UI readiness check failed for control {params.index}: {error}')
			if first is None or second is None:
				return ActionResult(
					error=(
						f'Control {params.index} did not satisfy the requested visible/enabled readiness conditions '
						'after waiting.'
					)
				)
			stable = first.model_dump(mode='json') == second.model_dump(mode='json')
			if not stable:
				return ActionResult(
					error=f'Control {params.index} changed during the {params.stable_ms}ms stability window.',
					metadata={
						'first': first.model_dump(mode='json'),
						'second': second.model_dump(mode='json'),
					},
				)
			message = f'Control {params.index} is visible, enabled, and stable after waiting {params.seconds:.3f} seconds.'
			return ActionResult(
				extracted_content=message,
				long_term_memory=message,
				metadata={'control': second.model_dump(mode='json'), 'stable_ms': params.stable_ms},
			)
	...
	def _register_done_action(self, output_model: type[T] | None, display_files_in_done_text: bool = True):
		if output_model is not None:
			...
			async def done(params: StructuredOutputAction, file_system: FileSystem, browser_session: BrowserSession):
				...
				# A negative completion is a valid bounded termination, not a success claim to verify.
				if params.success and self.completion_guard:
					guard_result = self.completion_guard(browser_session, json.dumps(output_dict, ensure_ascii=False))
					if isinstance(guard_result, Awaitable):
						guard_result = await guard_result
					if guard_result:
						return ActionResult(
							error=f'Completion blocked: {guard_result}',
							long_term_memory=f'Completion was blocked: {guard_result}',
						)
			...
			async def done(params: DoneAction, file_system: FileSystem, browser_session: BrowserSession):
				...
				if params.success and self.completion_guard:
					guard_result = self.completion_guard(browser_session, user_message)
					if isinstance(guard_result, Awaitable):
						guard_result = await guard_result
					if guard_result:
						return ActionResult(
