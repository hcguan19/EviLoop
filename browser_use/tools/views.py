...
class InspectControlsAction(BaseModel):
	visible_only: bool = Field(default=True, description='Return only visible controls.')
	max_controls: int = Field(default=100, ge=1, le=250, description='Maximum controls to return.')
	type: str | None = Field(
		default=None,
		description='Optional control type/tag filter, such as checkbox, radio, select, input, or textarea.',
	)
	index: int | None = Field(default=None, ge=1, description='Optional single control index to inspect.')


class SetControlValueAction(BaseModel):
	index: int = Field(ge=1, description='Control index from browser_state.')
	value: str | bool | float | list[str] = Field(
		description=(
			'Exact desired value. Use true/false for checkbox and radio controls, or a list of exact '
			'option texts/values for a multi-select control.'
		)
	)


class SetControlsByLabelsAction(BaseModel):
	"""Set visible checkbox/radio controls by exact accessible labels."""

	labels: list[str] = Field(
		min_length=1,
		max_length=50,
		description='Complete list of exact visible checkbox/radio labels to select.',
	)
	checked: bool = Field(default=True, description='Desired checked state for every matched control.')


class TransferControlValueAction(BaseModel):
	"""Copy an exact visible value between two indexed controls."""

	source_index: int = Field(ge=1, description='Source control index from browser_state.')
	target_index: int = Field(ge=1, description='Target control index from browser_state.')


class ControlAssignment(BaseModel):
	"""One exact control update in a verified form transaction."""

	index: int = Field(ge=1, description='Control index from browser_state.')
	value: str | bool | float | list[str] = Field(description='Exact desired value for this control.')


class SetFormValuesAction(BaseModel):
	"""A bounded batch of control assignments followed by required-field validation."""

	assignments: list[ControlAssignment] = Field(min_length=1, max_length=20)
	verify_required: bool = Field(
		default=True,
		description='After all assignments, verify that visible required form controls are valid.',
	)


class WaitForUiAction(BaseModel):
	"""Wait for a temporal constraint and optionally verify a live control afterward."""

	seconds: float = Field(default=0.0, ge=0.0, le=30.0, description='Minimum delay before verification.')
	index: int | None = Field(default=None, ge=1, description='Optional control index to verify after waiting.')
	require_visible: bool = Field(default=True, description='Require the control to be visible.')
	require_enabled: bool = Field(default=True, description='Require the control to be enabled.')
	stable_ms: int = Field(
		default=250,
		ge=0,
		le=2000,
		description='Require two identical control snapshots separated by this interval.',
	)
