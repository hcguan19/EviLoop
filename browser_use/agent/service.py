...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	def __init__(
		...
		visual_context_mode: Literal['vision_first', 'html_first', 'auto'] = 'vision_first',
		goal_aware_task_strategy: bool = False,
		...
		**kwargs,
		...
		if goal_aware_task_strategy:
			self.task = self._enhance_task_with_goal_aware_strategy(self.task)
		self.llm = llm
		...
		self.settings = AgentSettings(
			...
			goal_aware_task_strategy=goal_aware_task_strategy,
		...
		self._message_manager = MessageManager(
			...
			visual_context_mode=self.settings.visual_context_mode,
		)
	...
	def _enhance_task_with_goal_aware_strategy(self, task: str) -> str:
		"""Add a goal-aware execution strategy for search, shopping, and form tasks."""

		strategy = """

<goal_aware_task_strategy>
Use this strategy to improve task accuracy. Treat it as operational guidance, not as a replacement for the user request.

1. Build a structured goal before acting.
   In your thinking/memory, extract the task into these fields when applicable:
   - target_site_or_url
   - product_or_item_type
   - required_attributes
   - required_options such as color, size, style, flavor, scent, model, capacity, quantity
   - price_or_other_constraints
   - completion_condition

2. Rewrite search queries instead of searching the full user sentence.
   Prefer concise queries built from product_or_item_type + the strongest required attributes/options.
   If the first query is weak, try a different query that combines:
   - product type + required attribute
   - product type + option value
   - required attribute + option value
   Do not buy from unrelated results just because a result is clickable.

3. Score candidates before selecting one.
   On result lists, compare visible candidates against the structured goal:
   - product/item type match
   - required attributes match
   - required options appear available or likely available
   - price/constraint match
   Choose the highest-confidence candidate. If confidence is low, refine the query or inspect another result.

4. Verify before final submission or purchase.
   Before clicking a final submit, checkout, Buy Now, or equivalent action, create a strict checklist:
   - product_type: pass/fail/unknown
   - required_attributes: pass/fail/unknown
   - required_options: pass/fail/unknown
   - price_or_constraints: pass/fail/unknown
   - page_state_confirms_target: pass/fail/unknown
   Only submit or buy when every item is pass. Treat unknown as fail. If any requirement is missing or uncertain,
   do not submit yet. Recover first.

5. Recover from likely failure.
   If search results are irrelevant, change the query.
   If a product/page lacks a required option, go back and choose another candidate.
   If repeated actions do not change the page, change strategy or request screenshot/visual context if needed.
   If the final page reports a score/reward/result, read it exactly. A score below 0.99 is a failed partial match,
   not success. If steps remain after a score below 0.99, infer the missing constraint, go back/search again, reject
   the current product, and try a better candidate instead of calling done().

6. Keep AutoWebGLM-style failure memory.
   Maintain concise memory entries for:
   - rejected_products: product ids/titles that missed required constraints
   - avoid_queries: search queries that returned irrelevant results
   - failed_clicks: final actions that did not change the page
   - missing_constraints: attributes/options that caused partial reward
   Use this memory to avoid repeating the same failed action loop.
</goal_aware_task_strategy>
"""
		return task.rstrip() + strategy
