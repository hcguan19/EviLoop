...
class MessageManager:
	...
	def __init__(
		...
		self.visual_context_mode = visual_context_mode
	...
	def _dom_context_signals(self, browser_state_summary: BrowserStateSummary) -> dict[str, int | bool]:
		root = browser_state_summary.dom_state._root if browser_state_summary.dom_state else None
		stats = {
			'total_elements': 0,
		...
		def walk(node) -> None:
			if node is None:
				return
			stats['total_elements'] = int(stats['total_elements']) + 1
			if getattr(node, 'is_interactive', False):
				stats['interactive_elements'] = int(stats['interactive_elements']) + 1
			original_node = getattr(node, 'original_node', None)
			if original_node is not None:
				tag_name = (getattr(original_node, 'tag_name', '') or '').lower()
				attributes = getattr(original_node, 'attributes', None) or {}
				if tag_name == 'img':
					stats['images'] = int(stats['images']) + 1
				if (
					tag_name == 'canvas'
					or tag_name == 'input'
					and attributes.get('type', '').lower() in {'range', 'color'}
					or attributes.get('role', '').lower() == 'slider'
					or 'contenteditable' in attributes
				):
					stats['complex_controls'] = int(stats['complex_controls']) + 1
				node_value = getattr(original_node, 'node_value', '') or ''
				if isinstance(node_value, str):
					stats['text_chars'] = int(stats['text_chars']) + len(node_value.strip())
		...
		stats['is_dom_empty'] = total_elements < 10
		stats['is_skeleton_like'] = total_elements > 20 and text_chars < total_elements * 5
		stats['is_image_heavy'] = images >= 8 and text_chars < 1200
		return stats

	def _task_sounds_visual(self) -> bool:
		visual_terms = (
			'image',
			'picture',
			'photo',
		...
		return any(term in task for term in visual_terms)

	def _should_include_screenshot(
		self,
		browser_state_summary: BrowserStateSummary,
		use_vision: bool | Literal['auto'],
		include_screenshot_requested: bool,
		result: list[ActionResult] | None,
	) -> tuple[bool, str]:
		if use_vision is False:
			return False, 'vision disabled'
		if include_screenshot_requested:
			return True, 'screenshot explicitly requested by an action'
		if self.visual_context_mode == 'vision_first':
			return True, 'vision_first mode'

		signals = self._dom_context_signals(browser_state_summary)
		reasons: list[str] = []
		if signals['is_dom_empty']:
			reasons.append('DOM appears empty')
		if signals['is_skeleton_like']:
			reasons.append('DOM looks like a loading/skeleton page')
		if signals['is_image_heavy']:
			reasons.append('page is image-heavy with little text')
		if signals['complex_controls']:
			reasons.append(f'page contains {signals["complex_controls"]} visually ambiguous control(s)')
		if browser_state_summary.is_pdf_viewer:
			reasons.append('PDF viewer')
		if browser_state_summary.browser_errors:
			reasons.append('browser reported errors')
		if result and any(action_result.error for action_result in result):
			reasons.append('previous action returned an error')
		if self._task_sounds_visual():
			reasons.append('task likely needs visual judgement')

		if self.visual_context_mode == 'auto' and self.state.agent_history_items:
			recent_failures = sum(1 for item in self.state.agent_history_items[-3:] if item.error)
			if recent_failures >= 2:
				reasons.append('recent history suggests repeated errors')

		if reasons:
			return True, '; '.join(reasons)
		if use_vision == 'auto':
			return False, 'auto mode: no visual fallback signal detected'
		return False, f'{self.visual_context_mode} mode: DOM/HTML context is sufficient'
	...
	def create_state_messages(
		...
		# - True: Include screenshot according to visual_context_mode
		# - False: Never include screenshot
		include_screenshot, visual_context_reason = self._should_include_screenshot(
			browser_state_summary=browser_state_summary,
			use_vision=use_vision,
			include_screenshot_requested=include_screenshot_requested,
			result=result,
		)
		logger.debug(
			'Visual context decision: include_screenshot=%s mode=%s reason=%s',
			include_screenshot,
			self.visual_context_mode,
		...
		state_message = AgentMessagePrompt(
			...
			visual_context_note=(
				f'visual_context_mode={self.visual_context_mode}; screenshot_included={bool(screenshots)}; '
				f'reason={visual_context_reason}. Use the DOM/HTML interactive elements as the primary source of truth. '
				'Rely on screenshot only when visual layout, images, canvas, color, or missing DOM content matters.'
			),
